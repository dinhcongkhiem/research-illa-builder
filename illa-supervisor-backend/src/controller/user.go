...
func (controller *Controller) RetrieveUserByID(c *gin.Context) {
	...
	userID, errInGetUserID := controller.GetUserIDFromAuth(c)
	if errInGetUserID != nil {
		log.Printf("[ERROR] RetrieveUserByID - Failed to get userID from auth: %v", errInGetUserID)
		return
	}
	log.Printf("[INFO] RetrieveUserByID - Got userID: %v", userID)

	// retrieve
	user, err := controller.Storage.UserStorage.RetrieveByID(userID)
	if err != nil {
		log.Printf("[ERROR] RetrieveUserByID - Failed to retrieve user from storage: %v", err)
		controller.FeedbackBadRequest(c, ERROR_FLAG_CAN_NOT_GET_USER, "get user error: "+err.Error())
		return
	}
	log.Printf("[INFO] RetrieveUserByID - Successfully retrieved user: %+v", user)

	// ok, feedback
	response := model.NewGetUserByIDResponse(user)
	log.Printf("[INFO] RetrieveUserByID - Sending response: %+v", response)
