...
func (controller *Controller) CreateApp(c *gin.Context) {
	...
	log.Printf("[INFO] CreateApp - Got teamID: %v, userID: %v, userAuthToken: %v", teamID, userID, userAuthToken)

	if errInGetTeamID != nil || errInGetUserID != nil || errInGetAuthToken != nil {
		log.Printf("[ERROR] CreateApp - Failed to get parameters: %v, %v, %v", errInGetTeamID, errInGetUserID, errInGetAuthToken)
		return
	}
