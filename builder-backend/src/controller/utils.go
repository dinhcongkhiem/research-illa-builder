...
func (controller *Controller) GetMagicIntParamFromRequest(c *gin.Context, paramName string) (int, error) {
	...
	log.Printf("[INFO] GetMagicIntParamFromRequest - Received param '%s' with value: '%s'", paramName, paramValue)

	// check skipping id
	if intID, hitSkippingID := SKIPPING_MAGIC_ID[paramValue]; hitSkippingID {
		log.Printf("[INFO] GetMagicIntParamFromRequest - Found magic ID for param '%s': %d", paramName, intID)
		return intID, nil
	}

	// no hit, convert
	if len(paramValue) == 0 {
		log.Printf("[ERROR] GetMagicIntParamFromRequest - Missing required param: %s", paramName)
		controller.FeedbackBadRequest(c, ERROR_FLAG_VALIDATE_REQUEST_PARAM_FAILED, "please input param for request.")
		return 0, errors.New("input missing " + paramName + " field.")
	}

	paramValueInt := idconvertor.ConvertStringToInt(paramValue)
	log.Printf("[INFO] GetMagicIntParamFromRequest - Converted param '%s' from '%s' to int: %d", paramName, paramValue, paramValueInt)
...
func (controller *Controller) GetUserIDFromAuth(c *gin.Context) (int, error) {
	log.Printf("[INFO] GetUserIDFromAuth - Starting to get userID from context")
	
	// get request param
	userID, ok := c.Get("userID")
	if !ok {
		log.Printf("[ERROR] GetUserIDFromAuth - Failed to get userID from context")
	...
	log.Printf("[INFO] GetUserIDFromAuth - Got userID from context: %v (type: %T)", userID, userID)

	userIDInt, okAssert := userID.(int)
	if !okAssert {
		log.Printf("[ERROR] GetUserIDFromAuth - Failed to convert userID to int. Value: %v, Type: %T", userID, userID)
	...
	log.Printf("[INFO] GetUserIDFromAuth - Successfully got userID: %d", userIDInt)
