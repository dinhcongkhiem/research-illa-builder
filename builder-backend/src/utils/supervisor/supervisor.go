...
func (supervisor *Supervisor) ValidateUserAccount(token string) (bool, error) {
	log.Printf("[INFO] ValidateUserAccount - Starting validation with API endpoint: %s%s", supervisor.API, VALIDATE_USER_ACCOUNT)
	log.Printf("[INFO] ValidateUserAccount - Token to validate: %s", token)

	client := resty.New()
	requestToken := supervisor.Validator.GenerateValidateToken(token)
	log.Printf("[INFO] ValidateUserAccount - Generated request token: %s", requestToken)
	...
	if err != nil {
		log.Printf("[ERROR] ValidateUserAccount - HTTP request failed: %v", err)
		return false, errors.New("request illa supervisor failed: " + err.Error())
	}

	log.Printf("[INFO] ValidateUserAccount - Response status code: %d", resp.StatusCode())
	log.Printf("[INFO] ValidateUserAccount - Response body: %s", resp.String())

	if resp.StatusCode() != http.StatusOK {
		log.Printf("[ERROR] ValidateUserAccount - Validation failed with status code: %d", resp.StatusCode())
		return false, nil
	}

	log.Printf("[INFO] ValidateUserAccount - Validation successful")
	return true, nil
}
